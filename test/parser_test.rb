...
class ParserTest < MiniTest::Test
  ...
  def test_stripped_markers
    ...
    assert_equal({type: :text, content: ["Here's the basics of how text works in Gemtext:"]}, gmi[1])
    assert_equal({type: :list, :content=>["Long lines get wrapped by the client to fit the screen", "line2"]}, gmi[2])
    assert_equal({type: :verbatim, content: ["* Mercury", "* Gemini", "* Apollo"]}, gmi[10])
    assert_equal({type: :quote, content: ["* Mercury", "* Gemini", "* Apollo", "line2"]}, gmi[14])
